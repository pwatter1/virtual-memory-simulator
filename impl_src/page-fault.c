...
{	
	...
	pfn_t  victim_pfn = get_free_frame();	
	pcb_t *victim_pcb = rlt[victim_pfn].pcb; 
	vpn_t  victim_vpn = rlt[victim_pfn].vpn;
	 ...
	 * to the requesting process (which is nothing but the current process!). 
	 * Update the requesting process' page table. Set the page table entry as used and valid. 
	 * Also, if writing to the page, set the dirty bit. 
	 *

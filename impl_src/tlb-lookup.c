...
pfn_t tlb_lookup(vpn_t vpn, int write)
{
	 ...
	 *   else:
	 * 	mark the entry as unused, so we can kick it out the next time it is encountered. 
	 * This is called performing a clock-sweep, because we are mimicking a movement of time. 
